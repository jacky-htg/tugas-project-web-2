        ...
        <script>
          $(document).ready( function () {
            let myTable = $('#myTable').DataTable({
              ...
              buttons: [
                  {
                      text: '<button>Add New Pejabat</button>',
                      action: function ( e, dt, node, config ) {
                        window.location = "<?= base_url('pejabat');?>/create";
                      }
                  }
              ],
              'processing': true,
            ...
            myTable.on('click', 'i', function (e) {
              ...
              if (action === "update" ) {
                window.location = "<?= base_url('pejabat');?>/"+id+"/update"; 
              } else if (action === "delete") {
                if (confirm("Yakin ingin menghapus data # " + id)) {
                  $.ajax({
                    url: "<?= base_url('pejabat');?>/"+id+"/delete",
                    method: "POST",
                    success: function() {
                        myTable.ajax.reload();
                    }
